...
export function CreateProcessorModal({ isOpen, onClose, onSuccess }: CreateProcessorModalProps) {
  ...
  const [formData, setFormData] = useState<ProcessorFormData>({
    ...
    facilityAddress: '', latitude: '', longitude: '', facilityState: '', facilityCity: '', facilityPinCode: ''
  });
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      ...
      case 3:
        return !!(formData.facilityAddress && formData.facilityState && formData.facilityCity && formData.facilityPinCode);
      default:
        return true;
    }
  };
  ...
  const handleSubmit = () => {
    console.log('Creating processor:', formData);
  ...
  const updateFormData = (field: keyof ProcessorFormData, value: string) => {
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-700">Create the user account (this person will be the processor owner)</p>
                  </div>
                </div>
              </div>
        ...
        return (
          <Card>
            <CardHeader className="bg-gradient-to-r from-blue-50 to-blue-50">
              ...
              <p className="text-sm text-gray-600">Create a new facility for this processor</p>
            </CardHeader>
            <CardContent className="p-6 space-y-8">
              {/* Basic Information */}
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input id="facilityAddress" value={formData.facilityAddress} onChange={(e) => updateFormData('facilityAddress', e.target.value)} placeholder="Enter facility address" className="h-10" />
                  </div>
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <select id="facilityState" value={formData.facilityState} onChange={(e) => updateFormData('facilityState', e.target.value)} className="flex h-10 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900">
                        <option value="">Select State</option>
                        <option value="maharashtra">Maharashtra</option>
                        <option value="gujarat">Gujarat</option>
                      </select>
                    ...
                    <div>
                      ...
                      <select id="facilityCity" value={formData.facilityCity} onChange={(e) => updateFormData('facilityCity', e.target.value)} className="flex h-10 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900">
                        <option value="">Select City</option>
                        <option value="mumbai">Mumbai</option>
                        <option value="pune">Pune</option>
                      </select>
                    ...
                    <div>
                      <Label htmlFor="facilityPinCode">PIN Code *</Label>
                      <Input id="facilityPinCode" value={formData.facilityPinCode} onChange={(e) => updateFormData('facilityPinCode', e.target.value)} placeholder="Enter 6-digit PIN code" className="h-10" />
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="min-h-screen flex items-start justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[900px] my-8 relative">
          ...
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-xl font-bold text-gray-900 mb-1">Create New Processor</h1>
              <p className="text-sm text-gray-600">Follow the steps to create a complete processor profile</p>
            </div>
                ...
                <div key={step.id} className="flex items-center">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shadow transition-all duration-300 ${
                      isCompleted ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-blue-500/30' : 
                      isCurrent ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-blue-600/30' : 
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 font-medium">
            {currentStep === 3 ? 'Ready to create processor' : 'Fill required processor information'}
          </span>
          
          <Button
            onClick={handleNext}
            disabled={!validateStep(currentStep)}
            className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 px-4 py-2 text-sm rounded-lg shadow hover:shadow-blue-600/25 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
